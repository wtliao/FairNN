...
def kl_divergence(p, q):
    """
    :param p (tensor): data distribution 1 of one batch
    :param q (tensor): data distribution 2 of one batch
    :return: kl divergence of two distribution
    """
    mean_p = p.detach().numpy().mean()
    mean_q = q.detach().numpy().mean()
    var_p = np.var(p.detach().numpy())
    var_q = np.var(q.detach().numpy())
    return np.log(np.sqrt(var_p) / np.sqrt(var_q)) + (var_p + np.square(mean_p - mean_q)) / (2 * var_q) - 0.5
...
def Duplicating(batch, expected_value, name=''):
    if name == 'DP':
        batch_sort = torch.index_select(batch, 0, index=batch[:, 53].sort()[1])
        need_value = expected_value - len(batch_sort)
    elif name == 'PP':
        batch_sort = torch.index_select(batch, 0, index=batch[:, 53].sort()[1])
        need_value = len(batch_sort) - expected_value
    else:
        raise Exception("choose one community 'PP'or 'DP'!!")
...
def Skipping(batch, expected_value, name=''):
    if name == 'PN':
        batch_sort = torch.index_select(batch, 0, index=batch[:, 53].sort(descending=True)[1])
        drop_value = expected_value - len(batch_sort)
    elif name == 'DN':
        batch_sort = torch.index_select(batch, 0, index=batch[:, 53].sort(descending=True)[1])
        drop_value = len(batch_sort) - expected_value
    else:
        raise Exception("choose one community 'PN'or 'DN'")
...
def Preferential_sampling(label_pred, label_true, data):
    ...
    index_DP = torch.squeeze(torch.nonzero((dataset[:, 51] == 1) * (dataset[:, 50] == 1) == True))
    index_PP = torch.squeeze(torch.nonzero((dataset[:, 51] == 0) * (dataset[:, 50] == 1) == True))
    index_DN = torch.squeeze(torch.nonzero((dataset[:, 51] == 1) * (dataset[:, 50] == 0) == True))
    index_PN = torch.squeeze(torch.nonzero((dataset[:, 51] == 0) * (dataset[:, 50] == 0) == True))
    ...
    PP_batch_new = Duplicating(PP_batch, expected_amount[2], name='PP')  # PP Duplicating
    PN_batch_new = Skipping(PN_batch, expected_amount[3], name='PN')  # PN Skipping
    new_train_set = torch.cat((DP_batch_new, DN_batch_new, PP_batch_new, PN_batch_new), 0)[:, 0:53]
